...
while True:
    ...
    if keystate[pg.K_SPACE] and do_release:
        the_ship.shoot()
        do_release = False
    ...
    hits = pg.sprite.spritecollide(the_ship, meteors, True)
    if hits:
        while (True):
            clock.tick(FPS)
            ...
            end_game_text = font.render("YOU DIED", True, (255, 0, 0))
            end_game_text_rect = end_game_text.get_rect()
            end_game_text_rect.center = (screen.get_width() // 2, screen.get_height() // 2)
            ...
            score_text_rect.center = (score_text_rect.width // 2 + 20, score_text_rect.height // 2 + 30)
