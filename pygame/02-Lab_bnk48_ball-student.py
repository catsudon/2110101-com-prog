...
ball3_img = pg.image.load("source/BNK48/Panda_cc.png").convert_alpha()
...
while running:
    ...
    if running:
        ...
        font_name = pg.font.match_font('arial')  # กำหนดชื่อ Font
        font = pg.font.Font(font_name, 150)  # กำหนดขนาด font
        text_surface = font.render("6532180221", True, white) # กำหนด Text และ สี
        text_rect = text_surface.get_rect() # แปลง Surface เป็น object
        ...
        x1 = (ball1_rect.left + ball1_rect.right)/2
        y1 = (ball1_rect.top + ball1_rect.bottom)/2
        
        x2 = (ball2_rect.left + ball2_rect.right)/2
        y2 = (ball2_rect.top + ball2_rect.bottom)/2
        
        x3 = (ball3_rect.left + ball3_rect.right)/2
        y3 = (ball3_rect.top + ball3_rect.bottom)/2
        ...
        if(sqrt(dx**2 + dy**2) <= (150+100)/2):
            ball1_speed[0]*=-1;ball1_speed[1]*=-1
            ball2_speed[0]*=-1;ball2_speed[1]*=-1
            soundeffect.play()
